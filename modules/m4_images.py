...
def check(pdf_pages, detected_offset=0):
    ...
    # 使用 Markdown 列表罗列所有结果
    md_content = "### 📊 图表逻辑\n\n"
    
    for item in found_items:
        label, ch, num = item['label'], item['chapter'], item['num']
        ...
        # 罗列格式：状态 编号 - 备注（已删除页码显示）
        md_content += f"* {status_icon} **{item['full']}** — *{reason}*\n"
